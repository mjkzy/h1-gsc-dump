...
bm21_artillery_think()
{
    ...
    var_3["bm21_02"] = 2.2;
    var_3["bm21_03"] = 3.4;
    wait(var_3[self._id_9D45.teambalanced]);
...
sleepy_shack()
{
    ...
    if ( level.playercardbackground istouching( var_3 ) )
        return;
...
player_got_close()
{
    ...
    for (;;)
    {
        if ( distance( level.playercardbackground.origin, self.origin ) < 46 )
            return;
...
descriptions()
{
    ...
    for ( var_1 = 0; var_1 < var_0.size; var_1++ )
        var_0[var_1] thread print3dthreadzip( var_0[var_1].script_parentname );
}
...
bbc_voice()
{
    ...
    for (;;)
    {
        var_5 = distancesquared( level.playercardbackground.origin, self.origin );
        ...
        switch ( var_6 )
        {
            case "play_bbc":
                ...
                continue;
            case "done":
                var_2 thread _id_A5A4::_id_61FD( "play_bbc", randomfloatrange( 1, 2 ) );
                continue;
        }
    }
}
...
field_russian_think()
{
    ...
    {
        var_4 = getnode( self.script_parentname, "targetname" );
        ...
        var_7["stopper_2"] = 1.6;
        var_7["stopper_3"] = 1.8;
        wait(var_7[self.script_parentname]);
...
setup_sas_buddies()
{
    ...
    level.playercardbackground thread _id_A5A4::_id_A299( "shack_cleared" );
    ...
    if ( is_overlook_or_earlier_start() )
    {
        ...
        if ( self == level._id_6F7C )
        {
            if ( getdvarint( "use_old_burning_house" ) != 1 )
            {
                ...
                while ( distancesquared( level.playercardbackground.origin, level._id_6F7C.origin ) > 30000 )
                    waittillframeend;
...
_id_825D()
{
    ...
    level.playercardbackground._id_066A = 100;
}
...
street_walker_think()
{
    ...
    _id_A55F::_id_66FC( self._not_team );
    overlook_enemy_leaves_stealth();
}
...
ground_allied_forces()
{
    ...
    if ( !common_scripts\utility::_id_382E( "breach_complete" ) )
    {
        ...
        if ( isdefined( self.teambalanced ) && self.teambalanced == "hilltop_sniper_AI" )
...
price_checks_goal_for_noteworthy()
{
    ...
    for (;;)
    {
        ...
        if ( var_1.script_parentname == "signal_moveup" )
        {
...
overlook_turret_think()
{
    ...
    _id_A5A4::_id_27EF( randomfloatrange( 50, 55 ), _id_A5A4::_id_2A51 );
...
player_is_ads()
{
    return level.playercardbackground playerads() > 0.5;
}
...
physics_launch_think()
{
    ...
    for (;;)
    {
        ...
        if ( !isdefined( var_1.motiontrackerenabled ) )
            continue;

        if ( !issubstr( var_1.motiontrackerenabled, "vehicle" ) )
            continue;

        break;
    }

    var_7 = getent( self._not_team, "targetname" );
    var_8 = var_7.origin;
...
price_swap()
{
    for (;;)
    {
        if ( _id_A5A4::_id_6B91( level._id_6F7C geteye(), 0.7 ) )
        {
            wait 0.1;
            continue;
        }
...
take_out_pier_guy( var_0 )
{
    ...
    var_0.threatsightdelayfalloff = 35000;
...
blackout_flashlight_guy()
{
    ...
    level._id_3C61.ignoretriggers = 1;
    var_9 = [];
...
vip_rescue_dialogue( var_0 )
{
    ...
    level._id_6F7C.weapon_change = 5800;
    ...
    level._id_6F7C.weapon_change = 16;
...
blackout_flashlight_kill_player()
{
    ...
    for (;;)
    {
        if ( !bullettracepassed( self gettagorigin( "tag_flash" ), level.playercardbackground geteye(), 0, undefined ) )
        {
            wait 0.05;
            continue;
        }

        magicbullet( self.weapon_switch_invalid, self gettagorigin( "tag_flash" ), level.playercardbackground geteye() );
...
blackout_flashlight_death( var_0, var_1, var_2, var_3 )
{
    ...
    var_4 = getstartorigin( var_0.origin, var_0.angles, var_1 _id_A5A4::_id_3EF5( "fl_death" ) );
    var_2 _meth_8143();
    var_2 notify( "single anim", "end" );
    wait 0.2;
    var_0 thread _id_A510::_id_0C24( var_2, "fl_death" );

    if ( var_1.helmet == 50000 )
...
blind_guy_think()
{
    ...
    common_scripts\utility::_id_383D( "blind_trigger" + self.script_parentname );
    common_scripts\utility::_id_383D( "blind" + self.script_parentname );
    _id_A5A4::_id_07BE( _id_A5A4::_id_A099, "death" );
    level _id_A5A4::_id_075F( common_scripts\utility::_id_383F, "blind" + self.script_parentname );
    ...
    var_3 = var_1[self.script_parentname];
    var_4 = var_2[self.script_parentname];
    var_5 = var_0[self.script_parentname];
    var_6 = var_0[self.script_parentname] + "_death";

    if ( self.script_parentname == "corner_spawner" )
    {
    ...
    if ( self.script_parentname == "hide_spawner" )
    {
...
price_signals_on_arrival()
{
    ...
    for (;;)
    {
        ...
        if ( !isdefined( level._id_6F7C.node_relinquished ) )
            return;

        if ( level._id_6F7C.node_relinquished != self )
            return;

        var_0 = isdefined( level._id_6F7C.node_relinquished.script_parentname ) && level._id_6F7C.node_relinquished.script_parentname == "bottom_stairs_node";
...
node_notifies_on_arrival( var_0 )
{
    ...
    if ( self.node_relinquished != var_0 )
        return;
...
grenade_hint_logic()
{
    ...
    level.playercardbackground thread _id_A5A4::_id_2B4A( "grenade_launcher" );
}
...
detect_player_mantle()
{
    ...
    for (;;)
    {
        ...
        if ( level.playercardbackground _meth_83dc() )
...
shack_signal( var_0 )
{
    ...
    var_1 delete();
    wait 4;
    level.playercardbackground thread _id_A5A4::_id_2B4A( "claymore_plant" );

    for (;;)
    {
        if ( level.playercardbackground should_break_claymores() )
            break;
    ...
    wait 3.0;
    level.playercardbackground thread _id_A5A4::_id_2B4A( "claymore_placement" );
}
...
price_and_gaz_flash_hut()
{
    ...
    level._id_6F7C.groundentchanged = "flash_grenade";
    ...
    level._id_6F7C.groundentchanged = var_2;
    level._id_6F7C.grenadeammo = 0;
    wait 0.8;
...
player_in_house()
{
    ...
    objective_ring( 7 );
    level.playercardbackground thread _id_A5A4::_id_2B4A( "nvg" );
    common_scripts\utility::_id_3857( "player_in_house" );
    level.playercardbackground thread _id_A5A4::_id_2B4A( "disable_nvg" );
}
...
patrol_soon()
{
    ...
    level endon( "high_alert" );
    common_scripts\utility::_id_384A( "player_near_pier" );
    thread _id_A55F::_id_66FC( self._not_team );
}
...
hut_runner_think()
{
    ...
    for (;;)
    {
        ...
        self _meth_81aa( var_0.origin );
        self.goalradius = 128;
        self.pathlookaheaddist = 32;
        self.pathrandompercent = 32;
        wait 0.05;
    }
}
...
display_sniper_hint()
{
    if ( level.playercardbackground should_break_sniper_rifle_hint() )
        return;

    level._id_6F7C _id_A5A4::_id_2A32( "switch_sniper" );
    wait 4;
    level.playercardbackground thread _id_A5A4::_id_2B4A( "sniper_rifle" );
}
...
music_control()
{
    ...
    level notify( "next_music_track" );
    musicstop( 4 );
    level.playercardbackground playsound( "blackout_hurry_end" );
    wait 5.1;
...
cliff_reminder()
{
    ...
    for (;;)
    {
        if ( distance( level.playercardbackground.origin, level._id_6F7C.origin ) > 600 )
            over_here();
...
over_here()
{
    ...
    if ( distance( level.playercardbackground.origin, level._id_6F7C.origin ) > 600 && !common_scripts\utility::_id_382E( "mission_chatter" ) )
        level._id_6F7C _id_A5A4::_id_2A32( "get_down_here" );
}
...
shoot_magic_rocket( var_0 )
{
    ...
    if ( !isdefined( var_1.sprint_begin ) )
        ...
        var_2.units_per_second = var_1.sprint_begin;

    for (;;)
    {
        if ( isdefined( var_1.script_parentname ) && isdefined( int( var_1.script_parentname ) ) )
            thread common_scripts\_exploder::_id_3528( int( var_1.script_parentname ) );

        if ( !isdefined( var_1._not_team ) )
            break;

        var_3 = getent( var_1._not_team, "targetname" );
