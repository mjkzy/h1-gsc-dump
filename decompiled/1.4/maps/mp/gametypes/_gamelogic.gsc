...
updatewinstats( var_0 )
{
    ...
    if ( !hasdoneanycombat( var_0 ) && !( level.gametype == "infect" ) )
        return;
...
updatematchbonusscores( var_0 )
{
    ...
    if ( level.teambased )
    {
        ...
        foreach ( var_4 in level.players )
        {
            ...
            if ( !hasdoneanycombat( var_4 ) && !( level.gametype == "infect" ) )
                continue;
...
matchstarttimer_internal( var_0 )
{
    ...
    if ( level.xpscale > 1 && !( isdefined( level.ishorde ) && level.ishorde ) && !maps\mp\_utility::privatematch() && !( isdefined( level.iszombiegame ) && level.iszombiegame ) )
    {
        foreach ( var_2 in level.players )
...
outcomenotifytoallplayers( var_0, var_1, var_2, var_3 )
{
    foreach ( var_5 in level.players )
    {
        ...
        if ( isdefined( var_5.displaystatlossui ) && var_5.displaystatlossui )
            var_2 = game["end_reason"]["stat_loss_prevention"];

        if ( level.teambased )
        {
            var_5 thread maps\mp\gametypes\_hud_message::teamoutcomenotify( var_0, var_1, var_2, var_3 );
            continue;
        }

        var_5 thread maps\mp\gametypes\_hud_message::outcomenotify( var_0, var_2 );
    }
}
...
setweaponstat( var_0, var_1, var_2 )
{
    ...
    if ( var_2 != "timeInUse" && var_2 != "deaths" && !var_5 && !var_6 )
    {
        var_7 = var_0;
        var_0 = self getcurrentweapon();

        if ( var_0 != var_7 && maps\mp\_utility::iskillstreakweapon( var_0 ) )
            return;
    }
    ...
    if ( var_2 == "deaths" )
    {
        var_8 = maps\mp\_utility::getbaseweaponname( var_0 );

        if ( !maps\mp\_utility::iscacprimaryweapon( var_8 ) && !maps\mp\_utility::iscacsecondaryweapon( var_8 ) )
            return;

        var_9 = maps\mp\_utility::getweaponattachmentsbasenames( var_0 );
        maps\mp\gametypes\_persistence::incrementweaponstat( var_8, var_2, var_1 );
        maps\mp\_matchdata::logweaponstat( var_8, "deaths", var_1 );

        foreach ( var_11 in var_9 )
            maps\mp\gametypes\_persistence::incrementattachmentstat( var_11, var_2, var_1 );
    }
}
